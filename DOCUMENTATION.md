...
- `PEPPER_PRIMARY_URL`, `PEPPER_PRIMARY_KEY`
- `INTER_SERVICE_KEY`
...
> JWT maison, scrypt + pepper externe, refresh token rotation, Proof of Work, rate limiting.  
...
| Pepper | Pepper externe via `pepper-primary` (port 3098) |
...
- **Pepper** : secret externe récupéré via `GET /pepper` sur `pepper-primary` (port 3098)
  - Jamais stocké dans Auth-service ni PostgreSQL
  - *(Historique : double pepper HMAC retiré en 2026-06 — voir doc racine)*
...
| PEPPER_PRIMARY_URL, PEPPER_PRIMARY_KEY | Microservice pepper-primary |
...
# Démarrage (pepper-primary doit être lancé avant)
npm start
```
